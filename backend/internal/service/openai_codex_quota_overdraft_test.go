...
func TestCodexQuotaOverdraftDefersToAutomaticResetCredits(t *testing.T) {
	t.Cleanup(func() { SetCodexQuotaOverdraftEnabled(false) })
	SetCodexQuotaOverdraftEnabled(true)
	now := time.Now().UTC()
	reset := now.Add(time.Hour)
	...
	require.False(t, codexQuotaOverdraftSchedulingAllowed(account, now), "自动用卡尚未确认无卡时必须优先处理")
	account.Extra[OpenAIAutoResetCreditStateExtraKey] = &OpenAIAutoResetCreditState{
		Status:         OpenAIAutoResetStatusNoCredit,
		AvailableCount: 0,
		CheckedAt:      now.Format(time.RFC3339),
	}
	require.True(t, codexQuotaOverdraftSchedulingAllowed(account, now), "确认无重置卡后才允许进入透支兜底")
}
