...
func (c *CodexQuotaOverdraftCoordinator) observeAccount(account *Account, preferredModel string) {
	...
	now := c.currentTime()
	if codexQuotaOverdraftAutoResetPending(account, now) {
...
func (c *CodexQuotaOverdraftCoordinator) HandleQuota429(
	...
	if codexQuotaOverdraftAutoResetPending(account, c.currentTime()) {
		notifyOpenAIAutoReset(account.ID)
		return false
	}
...
// Automatic reset credits take precedence over overdraft. Once the reset
// workflow has conclusively reported no usable credit (or failed), overdraft
// may proceed as the fallback for that quota cycle.
func codexQuotaOverdraftAutoResetPending(account *Account, now time.Time) bool {
	config := ResolveOpenAIAutoResetCreditConfig(account)
	if !config.Enabled || account == nil {
		return false
	}
	...
	if (!has5h || utilization5h < config.Threshold5h) &&
		(!has7d || utilization7d < config.Threshold7d) {
		return false
	}
	...
	switch state.Status {
	case OpenAIAutoResetStatusNoCredit, OpenAIAutoResetStatusFailed:
		return false
	default:
		return true
	}
